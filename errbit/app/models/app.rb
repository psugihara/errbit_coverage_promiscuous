class App
  ...
  def self.report_error!(*args)
    Rails.logger.debug 'def self.report_error!(*args)'
  ...
  def report_error!(hash)
    Rails.logger.debug 'def self.report_error!(hash)'
  ...
  def find_or_create_err!(attrs)
    ...
    if err.nil?
      problem_attrs = attrs.merge(app: self)
      problem = Problem.create!(problem_attrs)
      problem.cache_app_attributes
      attrs.merge!(problem: problem)
      err = Err.create!(attrs)
    end
